...
setVerifying(true)
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_PORT}/api/auth/login`, {
          method: "POST",
          headers: {
  ...
  return (
    ...
    <div className='bg-white w-10/12 md:w-6/12 lg:w-4/12 p-8 rounded'>
        ...
        <TextField
          ...
          />
       <button onClick={()=>setShowPw(!showPw)} className='text-right mt-1 text-sm   float-right'>{showPw? 'Hide Password' : 'Show Password'}</button>
         <button
         disabled = {!username && !password}
         className='mt-6 w-full bg-emerald-500 text-white rounded p-2'
          onClick={login}
          >{verifying ? 'VERIFYING..': 'LOGIN'}</button> 
