    ...
    if (!email?.includes("@")) {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_PORT}/api/subscription`,
          {
            method: "POST",
            headers: {
  ...
  return (
    <div className="mt-10 md:mt-24 bg-black  border-b border-gray-800">
      <div className=" px-5 md:px-0 md:w-10/12 m-auto py-14 lg:flex justify-between">
        ...
        {done ? (
          <button className="border flex items-center border-white mt-6 lg:mt-0 text-white w-full lg:w-fit h-fit py-2 px-5 bg-emerald-500 ">
            SUBSCRIBED
